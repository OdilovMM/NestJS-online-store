...
export class ProductsService {
    ...
    async findAll(query: any): Promise<{ products: any[]; totalProducts; limit }> {
        ...
        const queryBuilder = dataSource
            ...
            .addSelect(['COUNT(review.id) AS reviewCount', 'AVG(review.ratings)::numeric(10,2) AS avgRating'])
    ...
    async findOne(id: number): Promise<ProductEntity> {
        ...
        if (!product) throw new NotFoundException(`Product not found with that id of ${id}`);
        return product;
    }
    ...
    async remove(id: number) {
        ...
        if (order) throw new BadRequestException('Product is in the order');
